...
test two cross-attention pose encoder variants. In both, the MTR agent feature
acts as a single query vector (projected to 256D) and the 11 pose frames act as
keys/values (each 144D projected to 256D). The attended output is concatenated
with the agent feature.
...
\citep{vaswani2017attention} is added to the projected pose keys/values before attention,
injecting the frame index $t \in \{0, \ldots, 10\}$ as fixed (non-learned) sinusoidal
features. This adds temporal ordering at zero extra parameter cost and tests whether
