...
the map ablation: GRU+geo+map (H6_geo_only_with_map) vs no-pose+map baseline
(H6_no_pose_with_map).

Discovers tiers automatically by computing per-scene minADE improvement,
then renders one PNG per scene under qualitatives_with_map/.

Layout: single panel with
  - past trajectory (gray)
  - GT future (black dashed)
  - no-pose+map modes (red, faded) + top mode (red, solid)
  - GRU+geo+map modes (blue, faded) + top mode (blue, solid)
"""
...
POSE_PKL  = "output/waymo/mtr+pose_data_geo_only_with_map/H6_geo_only_with_map/eval/eval_with_train/result.pkl"
NPOSE_PKL = "output/waymo/mtr+pose_data_no_pose_with_map/H6_no_pose_with_map/eval/eval_with_train/result.pkl"
OUT_DIR   = "qualitatives_with_map"
PAST_END  = 11
VALID_COL = 9
XY        = [0, 1]

# ── scene-level ADE helpers ─────────────────────────────────────────────────

def min_ade(pred, gt, valid):
    """minADE over K modes; pred (K,T,2), gt (T,2), valid (T,) bool."""
    if valid.sum() == 0:
        return float('nan')
...
# ── discover best scenes ─────────────────────────────────────────────────────

def discover_scenes(pose_dict, npose_dict):
    """
    Returns list of (tier_label, sid, oid, rel_imp, abs_imp) sorted by rel_imp.
    Tiers:
      tier1 — rel ≥ 50%  (pose+map strongly outperforms no-pose+map)
      tier2 — abs Δ ≥ 0.3 m  (large absolute improvement)
      tier3 — rel ≥ 20%   (solid relative improvement)
    """
    ...
    for key in common:
        ...
        abs_imp = n_ade - p_ade          # positive = pose better
        rel_imp = abs_imp / n_ade        # fraction
    ...
    for sid, oid, rel, absi, p_ade, n_ade in records:
        k = (sid, oid)
        if rel >= 0.50 and absi >= 0.10:           # large relative AND non-trivial absolute
    ...
    print(f"\n=== Tier 1 (≥50% relative improvement, top {len(tier1)}) ===")
    for s, o, r, a in tier1:
        print(f"  ({s!r:30s}, {o!r:6s})  rel={r*100:+.1f}%  abs={a:+.4f}")

    print(f"\n=== Tier 2 (|Δ| ≥ 0.30 m, top {len(tier2)}) ===")
    for s, o, r, a in tier2:
        print(f"  ({s!r:30s}, {o!r:6s})  rel={r*100:+.1f}%  abs={a:+.4f}")

    print(f"\n=== Tier 3 (≥20% relative, top {len(tier3)}) ===")
...
def render_scene(ax, p_item, n_item, title):
    ...
    for k in range(n_pred.shape[0]):
        ...
        if k == top_n:
            ax.plot(vxy[:, 0], vxy[:, 1], color='#d62728', alpha=1.0,
                    linewidth=1.6, zorder=4, label='No-pose+map')
    ...
    for k in range(p_pred.shape[0]):
        ...
        if k == top_p:
            ax.plot(vxy[:, 0], vxy[:, 1], color='#1f77b4', alpha=1.0,
                    linewidth=1.6, zorder=4, label='GRU+geo+map (ours)')
    ...
    ax.set_title(
        f"{title}\n"
        f"Pose+map ADE={p_ade:.3f}  No-pose+map ADE={n_ade:.3f}  "
        f"Δ={delta:+.3f} ({delta/n_ade*100:+.1f}%)",
        fontsize=7, pad=3
    )
...
def main():
    ...
    print(f"  pose+map:    {len(pose_dict)} items")
    print(f"  no-pose+map: {len(npose_dict)} items")
