...
outperforms all other configurations. We further show that cross-attention pose encoding
without temporal positional embeddings provides essentially \emph{no} improvement over the
baseline (0.6765 vs.\ 0.6745), whereas a GRU encoder---which preserves sequential gait
dynamics through its temporal inductive bias---achieves the full 7.6\% gain.
These results reveal two underappreciated design principles: (1) rotation-space
supervision is more informative than joint-space supervision for encoding gait into
trajectory-useful features; and (2) temporal ordering of body poses is essential, as
even a strong cross-attention architecture fails when pose frames are treated as an
unordered set.
\end{abstract}
...
\begin{itemize}
  ...
  \item \textbf{The GRU's temporal inductive bias is essential}. Replacing the GRU
        with a cross-attention encoder---where the agent feature queries 11
        pose-frame tokens without positional encoding---yields no improvement over
        the trajectory-only baseline (0.6765 vs.\ 0.6745). The GRU processes
        frames sequentially; geodesic supervision shapes this sequential integration
        into an orientation-aware representation that cross-attention cannot
        replicate when frame order is discarded.
\end{itemize}
...
Table~\ref{tab:temporal} compares the GRU encoder against the cross-attention
encoder (Section~\ref{sec:cross_attn}), both with geodesic-only supervision.

\begin{table}[t]
\caption{Temporal encoding ablation: GRU vs.\ cross-attention (no temporal PE).}
\label{tab:temporal}
\begin{center}
\begin{tabular}{lccc}
\toprule
Pose Encoder & Loss & \minADE $\downarrow$ & $\Delta$ vs.\ baseline \\
\midrule
None (no pose)             & ---      & 0.6745 & --- \\
Cross-attention (no PE)    & geo only & 0.6765 & $+$0.3\% (worse) \\
\rowcolor{bestrow}
GRU (sequential)           & geo only & \textbf{0.6231} & $-$7.6\% \\
\bottomrule
\end{tabular}
\end{center}
\end{table}

Cross-attention without temporal positional encoding gives 0.6765---essentially
identical to the no-pose baseline (0.6745), and dramatically worse than the GRU
(0.6231). The GRU gap is 0.053 in absolute \minADE.

This result is mechanistically informative: cross-attention can identify
\emph{which} pose frames are informative for a given agent, but without positional
encoding it cannot represent \emph{how} poses change over time. Geodesic supervision
requires this temporal context to be useful---it shapes the encoder to track the
evolution of joint orientations (\eg, gait phase transitions), not just their
presence. The GRU's sequential inductive bias encodes exactly this temporal
evolution. Adding sinusoidal positional encodings to the cross-attention variant
is a natural next step (Section~\ref{sec:limitations}).
...
\caption{%
  ...
  Validation \minADE over 30 epochs for \mtrpose (geo only, blue) and the
  trajectory-only baseline (gray dashed).
  The pose-conditioned model converges earlier (best at epoch~20) with
  slightly higher epoch-to-epoch variance; the baseline plateaus more
  smoothly at a higher \minADE.
}
\label{fig:training_curves}
\end{figure}

Figure~\ref{fig:training_curves} shows validation \minADE over 30 training epochs
for the pose-conditioned and trajectory-only models. \mtrpose (geo only) reaches
its best epoch earlier (\textit{epoch 20}) and with higher variance across epochs,
consistent with the additional noise introduced by the SMPL pose matching signal.
The trajectory-only baseline converges more smoothly but plateaus at a higher
\minADE.
...
The GRU hidden state at time $t$ integrates all pose frames $\mathbf{p}_1, \ldots, \mathbf{p}_t$.
With geodesic supervision, the loss penalizes the \emph{predicted future rotation}
which is decoded from the hidden state. Backpropagation therefore shapes $\mathbf{h}_T$
to preserve the temporal structure of rotation changes---\ie, gait phase, turning
direction, walking speed---which are precisely the features useful for predicting
where a pedestrian will walk next. Cross-attention without positional encoding
produces a hidden state that is rotation-aware but \emph{temporally unordered}:
it knows what rotations occurred but not in what sequence, losing the gait
dynamics that make pose useful for trajectory prediction.
...
\textbf{Cross-attention with positional encoding.} Our H5 ablation tests
cross-attention \emph{without} temporal positional embeddings. Adding sinusoidal
or learned PE could recover some or all of the GRU advantage. This variant is a
...
representations. Through a systematic ablation of 13 runs on the Waymo Open
...
loss;
(2) the GRU's sequential temporal encoding is essential---cross-attention without
positional encoding provides no improvement over the no-pose baseline;
...
Table~\ref{tab:full} reports all 13 experimental runs including invalid (zero
