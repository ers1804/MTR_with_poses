...
positional encoding provides no improvement (0.6765 vs.\ baseline 0.6745),
while cross-attention \emph{with} sinusoidal positional encoding recovers 79\% of the GRU's
advantage (0.6337, $-$6.0\%).
...
A map-context ablation (2$\times$2 design: $\pm$map $\times$ $\pm$pose) reveals that
HD map features reduce \minADE by \textbf{27.7\%} (0.6745 $\to$ 0.4880) without any
pose conditioning, dwarfing the pose-only gain. When map context is available, the
incremental pose benefit shrinks from 7.6\% to 1.7\% (0.4880 $\to$ 0.4797), because
body orientation and map-encoded routing constraints share information about pedestrian
heading---making the two signals partially redundant.
Finally, to address the small-dataset bottleneck of our 579-scene subset, we pretrain the
\mtr backbone on the full Waymo pedestrian set (231k examples across 487k scenes) and
fine-tune both the no-pose and pose-conditioned variants on the original subset using a
\emph{residual zero-init} pose fuser that preserves the pretrained backbone at fine-tune
init. Pretraining yields a $\sim$\textbf{21\%} \minADE reduction \emph{independent} of pose
(0.4880 $\to$ 0.3861 no-pose; 0.4797 $\to$ 0.3749 with pose), and pose still contributes
\textbf{2.9\%} on top of the strong pretrained prior---slightly more than the 1.7\% benefit
observed without pretraining. Pose's contribution becomes more visible against a strong
trajectory prior than against noisy from-scratch features.
\end{abstract}
...
Our experiments yield six principal findings:
...
\begin{itemize}
  ...
  \item \textbf{Temporal ordering dominates; sequential integration adds incrementally}.
        ...
        Adding sinusoidal positional encoding recovers 79\% of the GRU's advantage (0.6337, $-$6.0\%).
        The GRU's causal sequential integration achieves the full $-$7.6\% gain (0.6231),
        providing a further 1.6\% over cross-attention with PE.
  \item \textbf{Improvement is driven by past pose encoding, not future prediction quality.}
        Using 30fps AMASS pseudo-ground-truth future poses (99\% step coverage)
        versus zero future supervision gives nearly identical trajectory accuracy
        (0.6567 vs.\ 0.6532, $\Delta{=}0.5\%$), confirming the benefit arises
        entirely from the \emph{past} GRU encoder.
  \item \textbf{HD map context dominates pose; the two signals are partially redundant.}
        A 2$\times$2 ablation ($\pm$map $\times$ $\pm$pose) shows that adding HD map
        polylines yields $-$27.7\% \minADE without any pose conditioning.
        The pose gain shrinks from 7.6\% (no map) to only 1.7\% (with map), because
        body orientation and map-encoded routing constraints share information about
        pedestrian heading.
  \item \textbf{Backbone pretraining on full Waymo dominates architectural choices, and
        amplifies the pose benefit.} Pretraining the trajectory-only backbone on the full
        Waymo pedestrian set (231k examples, 487k scenes) and fine-tuning the pose-augmented
        architecture on the 579-scene subset yields $-$21.9\% \minADE (0.4797 $\to$ 0.3749).
        A controlled 2$\times$2 ablation ($\pm$pretrain $\times$ $\pm$pose) attributes
        $\sim$21\% to pretraining \emph{independent} of pose, and 2.9\% to pose on top of
        the pretrained backbone---a larger pose contribution than the 1.7\% observed
        without pretraining. The 579-scene subset was the binding constraint; once removed,
        pose's incremental signal becomes more visible against the strong trajectory prior.
\end{itemize}
...
richer, anatomically-grounded representation—applied to the Waymo Open Motion
Dataset within the state-of-the-art MTR transformer framework.
We conduct the first systematic study of pose supervision type (geodesic vs.\
MPJPE vs.\ GMM NLL), loss weight sensitivity, temporal encoding architecture
(GRU vs.\ cross-attention with and without positional encoding), and future
pose quality for joint trajectory--pose learning.
...
\textbf{H5 (no positional encoding).} No temporal information is injected before attention:
...
\textbf{H5b (sinusoidal positional encoding).} Standard sinusoidal positional encoding
...
Adding sinusoidal positional encoding (cross-attention + PE) recovers \textbf{79\%}
of the GRU's advantage, reaching \minADE $= 0.6337$ ($-6.0\%$).
The GRU achieves 0.6231 ($-7.6\%$), a further 1.6\% improvement over cross-attention with PE.

The progression reveals a clean decomposition: (1)~temporal \emph{ordering}---knowing
\emph{when} each frame occurred---is the dominant requirement, contributing 79\% of
...
\caption{%
  ...
  recovering 79\% of the GRU's advantage.
...
context interacts with pose conditioning, we enable HD map loading
(\texttt{WITHOUT\_HDMAP: False}) and train the 2$\times$2 factorial design:
$\pm$map $\times$ $\pm$pose (GRU+geo at $w=0.1$).

\begin{table}[t]
\caption{Map $\times$ pose ablation. All runs: 30 epochs, same training subset.
$\Delta$ is relative change vs.\ the no-pose, no-map baseline (0.6745).}
\label{tab:map_ablation}
\begin{center}
\begin{tabular}{llcc}
\toprule
Pose Enc. & Map & \minADE $\downarrow$ & $\Delta$ vs.\ baseline \\
\midrule
None  & No map  & 0.6745 & --- \\
None  & With map & 0.4880 & $-$27.7\% \\
GRU+geo & No map  & 0.6231 & $-$7.6\% \\
\rowcolor{bestrow}
GRU+geo & With map & \textbf{0.4797} & $-$28.9\% \\
\bottomrule
\end{tabular}
\end{center}
\end{table}

Table~\ref{tab:map_ablation} reveals two findings.
\emph{First}, HD map context is the dominant signal: adding map features alone (no pose)
reduces \minADE by \textbf{27.7\%}, far exceeding the 7.6\% gain from pose alone.
Pedestrian routing---which paths are available, which crosswalks are nearby---is
encoded directly in the map, and this spatial prior substantially reduces prediction
uncertainty.
\emph{Second}, when map context is available, the incremental benefit of pose conditioning
shrinks from 7.6\% to \textbf{1.7\%} (0.4880 $\to$ 0.4797, the best from-scratch result).
We attribute this to partial information redundancy: body orientation encodes where
the pedestrian is heading, which the map's routing constraints already constrain.
Without a map, pose fills the gap by providing an orientation proxy; with a map,
most of that gap is already closed, leaving only 1.7\% residual gain for pose.
...
(${\approx}486$k scenarios). To test whether dataset scale is the binding constraint
...
with the same hyperparameters as before. Naively replacing the backbone's agent feature
with the output of a randomly initialized pose fuser would corrupt the pretrained
representation at fine-tune iter 0. We address this with a \emph{residual zero-init} pose
fuser: the fuser's output is \emph{added} to the backbone feature (rather than replacing
it), and its final linear layer is initialized to zero. At fine-tune iter 0 the pose pathway
contributes exactly zero, so the pretrained backbone passes through unchanged; the pose
encoder warms up additively without disrupting the established trajectory features.

\begin{table}[t]
\caption{Pretrain $\times$ pose ablation. All runs include HD map and use 30-epoch fine-tuning
on the 579-scene subset. Pretrained variants load the full-Waymo backbone via residual
zero-init pose fusion.}
\label{tab:pretrain_ablation}
\begin{center}
\begin{tabular}{llccc}
\toprule
Pretrain & Pose Enc. & \minADE $\downarrow$ & \minFDE $\downarrow$ & $\Delta$ \\
\midrule
None  & ---       & 0.4880 & 1.0432 & --- \\
None  & GRU+geo   & 0.4797 & 1.0284 & $-$1.7\% \\
Full Waymo & ---       & 0.3861 & 0.8166 & $-$20.9\% \\
\rowcolor{bestrow}
Full Waymo & GRU+geo   & \textbf{0.3749} & \textbf{0.7769} & $-$23.2\% \\
\bottomrule
\end{tabular}
\end{center}
\end{table}

Table~\ref{tab:pretrain_ablation} gives a clean decomposition: pretraining accounts for an
approximately constant $-21\%$ \minADE reduction \emph{independent} of pose ($-20.9\%$ no-pose;
$-21.9\%$ pose). On top of the pretrained backbone, pose conditioning contributes a further
$\textbf{2.9\%}$ \minADE and $\textbf{4.9\%}$ \minFDE. Pose's incremental benefit
\emph{increases} from 1.7\% (from scratch) to 2.9\% (pretrained)---suggesting pose's
orientation signal is more informative paired with strong trajectory features than when
both must be learned simultaneously from a small dataset.
...
\caption{%
  ...
  predicted modes faded; top-scoring mode highlighted (red: no-pose; blue: geo+pose).
  Per-scene \minADE in titles.
...
no learned parameters, is sufficient to recover 79\% of the GRU's gain ($-$6.0\% vs.\ $-$7.6\%).
...
temporal structure.
\emph{Second}, the GRU's causal \emph{sequential integration} adds an incremental 1.6\%.
...
Importantly, the GRU and cross-attention+PE encoders have nearly identical parameter
counts (${\approx}505$K vs.\ ${\approx}497$K respectively, since sinusoidal PE is
parameter-free). The GRU's advantage is therefore purely an inductive bias effect,
...
(Sections~\ref{sec:main_result}--\ref{sec:map_ablation}) are conducted on a 579-scene
subset of Waymo. Section~\ref{sec:pretrain_ablation} addresses this partially via full-Waymo
pretraining, but fine-tuning still uses the same 579-scene subset---the pose-conditioned
fine-tune does not see the full dataset. The 2$\times$2 pretrain $\times$ pose result shows
that pose's incremental benefit \emph{increases} (from 1.7\% to 2.9\%) when the backbone
already encodes a strong trajectory prior, which is suggestive of how pose may behave at
full scale, but a definitive answer requires training the pose-conditioned model on the
full Waymo set, which our compute budget did not permit.
...
\textbf{Map context reduces pose benefit.} Our main ablations (Sections~\ref{sec:main_result}--\ref{sec:temporal_ablation}) use zero-padded map features.
A follow-up 2$\times$2 ablation (Section~\ref{sec:map_ablation}) shows that HD map context
alone yields $-$27.7\% \minADE, dwarfing the $-$7.6\% pose gain. With map, pose adds only
1.7\% on top. While pose conditioning remains beneficial in map-equipped settings, the
relative contribution is substantially smaller---a practitioner deploying \mtrpose in a
full map-enabled system should expect modest, not large, gains from pose conditioning.
...
\textbf{Single-seed results.} All runs use a single random seed. Epoch-to-epoch
variance in our training curves (particularly the LR-decay spike at epoch 25 in
the H5b run, where minADE jumps to 0.7420 before recovering) suggests non-trivial
training noise. We report best-checkpoint minADE, which may slightly overfit to
the noise of a single run. Repeating key runs with multiple seeds would tighten the
confidence interval on the reported improvements.
...
representations. Through a systematic ablation of 19 runs on the Waymo Open
...
sinusoidal PE recovers 79\% of the GRU's advantage (0.6337, $-$6.0\%); the GRU's
...
quality;
(4) HD map context is the dominant signal ($-$27.7\% minADE without any pose), and
when map features are available the incremental pose benefit shrinks from 7.6\% to
1.7\%, revealing partial redundancy between body orientation and map-encoded routing
constraints;
(5) full-Waymo backbone pretraining (231k pedestrian examples, 487k scenes) yields a
$\sim$21\% \minADE reduction \emph{independent} of pose, dwarfing every architectural
ablation in this work; on top of the pretrained backbone, pose conditioning still
contributes 2.9\% \minADE and 4.9\% \minFDE---a larger pose benefit than at the
from-scratch scale (1.7\%), suggesting pose's orientation signal is more visible
against a strong trajectory prior. A residual zero-init pose fuser is the key
architectural ingredient: it preserves the pretrained backbone exactly at fine-tune
init while allowing the pose pathway to warm up additively.

These findings suggest that rotation-space supervision, temporal ordering, and
backbone pretraining are underappreciated design choices in pose-conditioned forecasting,
and that the value of pose conditioning is modulated by both available map context and
the strength of the trajectory prior the backbone has learned. We hope the systematic
ablation methodology contributes to more principled design of multi-modal pedestrian
prediction systems.
...
\begin{itemize}
  ...
  \item Cross-attention pose encoder (H5/H5b): pose projection 144$\to$256, MultiheadAttention(embed\_dim=256, num\_heads=8, dropout=0.1, batch\_first=True); ${\approx}497$K parameters total including fusion MLP
  \item GRU pose encoder: GRU(input=144, hidden=256, layers=1); ${\approx}505$K parameters total including fusion MLP
  \item Batch size: 10
...
Table~\ref{tab:full} reports all 19 experimental runs including invalid (zero
future GT) baselines for completeness.

\begin{table}[h]
\caption{Complete experimental results. Runs 001--002 are invalid (zero future GT)
and excluded from analysis. Baseline for $\Delta$ is run 004 (no pose, no map).
Run 017 is the H7 backbone pretraining only; runs 018--019 fine-tune from H7 on the
579-scene subset.}
\label{tab:full}
\begin{center}
\small
...
\toprule
Run & Tag & Encoder & Losses / Map / Pretrain ($w$) & \minADE & $\Delta$ \\
\midrule
...
\midrule
017 & H7 pretrain     & --- & traj only + map (full Waymo, 231k peds) & --- & \textit{ckpt only} \\
019 & H9 finetune     & --- & traj + map, H7 pretrain & 0.3861 & $-$42.8\% \\
\rowcolor{bestrow}
018 & H8 finetune     & GRU & geo+gmm (0.1) + map, H7 pretrain & \textbf{0.3749} & $-$44.4\% \\
\bottomrule
\end{tabular}
\end{center}
\end{table}
