...
shrinks from 7.6\% to \textbf{1.7\%} (0.4880 $\to$ 0.4797, still the new overall best).
...
\textbf{Single-seed results.} All 14 runs use a single random seed. Epoch-to-epoch
...
representations. Through a systematic ablation of 16 runs on the Waymo Open
...
underappreciated design choices in pose-conditioned forecasting, and that the value
of pose conditioning is modulated by available map context. We hope the
...
Table~\ref{tab:full} reports all 16 experimental runs including invalid (zero
...
\toprule
Run & Tag & Encoder & Losses / Map ($w$) & \minADE & $\Delta$ \\
\midrule
...
\rowcolor{bestrow}
016 & H6 geo+map      & GRU & geo+gmm (0.1) + map  & \textbf{0.4797} & $-$28.9\% \\
\bottomrule
\end{tabular}
\end{center}
\end{table}
